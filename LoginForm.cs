    ...
    public partial class LoginForm : Form
    {
        ...
        private void Button_login_Click(object sender, EventArgs e)
        {
            ...
            else 
            {
                ...
                string selectquery = "SELECT * FROM `employee` WHERE `username` = @usn AND `password`=@pswd";
                MySqlCommand command = new MySqlCommand(selectquery, connect.GetConnection());
                command.Parameters.Add("@usn", MySqlDbType.VarChar).Value = TextBox_username.Text;
                command.Parameters.Add("@pswd", MySqlDbType.VarChar).Value = TextBox_password.Text;
